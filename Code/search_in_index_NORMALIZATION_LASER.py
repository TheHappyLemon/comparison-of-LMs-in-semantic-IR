import h5py
import logging
from constants import PATH_LOGS, PATH_DATASET, CHUNK_SIZE, PATH_SEARCH_FLATIP_NORMALIZED_LASER
import faiss
import numpy as np
import csv
import os
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(PATH_LOGS + "search_NORMALIZED_LASER.log")
    ]
)
...
if __name__ == '__main__':
    ...
    hdf5_file = PATH_DATASET + "embeddings_old.hdf5"
    kNN       = [1, 5, 10, 20]
    datasets  = get_dataset_names(hdf5_file, "LASER")
    for index_type in index_types:
        
        if index_type == 'FlatIP':
            path_search_result = PATH_SEARCH_FLATIP_NORMALIZED_LASER
        else:
